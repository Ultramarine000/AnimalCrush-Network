Shader "Hidden/UnderwaterImageEffect"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            #define M_PI 3.1415936535897932384626433832795
            ...
            v2f vert (appdata v)
            {
                ...
                o.scrPos = ComputeScreenPos(o.vertex);//获取屏幕坐标
                o.uv = v.uv;
                return o;
            }
            ...
            fixed4 frag (v2f i) : COLOR
            {
                ...
                float3 spos = float3(i.scrPos.x, i.scrPos.y, 0) * _NoiseFrequency; //屏幕坐标
                spos.z += _Time.x * _NoiseSpeed;
                float noise = _NoiseScale * ((snoise(spos) + 1) / 2);//snoise()返回值【-1,1】，处理成【0,1】
                float4 noiseToDirection = float4(cos(noise * 2 * M_PI), sin(noise * 2 * M_PI), 0, 0);
                //fixed4 col = fixed4(noise,noise,noise,1);//灰色流动(p4)
